...
class Repository {
  public:
   ...
   /**
    * @brief Initializes a repository at a given fs location
    * @param force Create even if target is not empty
    * @return True if a lock was held or acquired.
    */
    static Repository initialize(const boost::filesystem::path& path,
                                 bool force = false);
    ...
    /**
     * @brief Lock this repository against concurrent writing.
     * @param force Release any lock, even if owned by another process.
     * @return True if the repository is unlocked at the end of the call, even
     * if it wasn't locked before.
     */
